...
#region Properties
    ...
    // Allow multiple spouses to be stored for a person (very Biblical).
    public List<Person> Spouses = new();
    public bool Married => Spouses.Count > 0;
#endregion Properties

#region Methods
    public void WriteToConsole() => WriteLine($"{Name} was born on a {Born:dddd}.");
    ...
    public static void Marry(Person p1, Person p2)
    {
        ...
        if (p1.Spouses.Contains(p2) || p2.Spouses.Contains(p1)) { throw new ArgumentException(string.Format("{0} is already married to {1}.", arg0: p1.Name, arg1: p2.Name)); }
        p1.Spouses.Add(p2);
        p2.Spouses.Add(p1);
    }

    // Instance method to marry another person
    public void Marry(Person partner) => Marry(this, partner); // "this" is the current person.
    ...
    public void OutputSpouses()
    {
        if (Married)
        {
            ...
            foreach (Person spouse in Spouses) { WriteLine($"  {spouse.Name}"); }
        }
        else { WriteLine($"{Name} is a singleton."); }
    }
    ...
    /// <returns>A Person object that is the child of Parent 1 and Paremt 2.</returns>
    ...
    public static Person Procreate(Person p1, Person p2)
    {
        ArgumentNullException.ThrowIfNull(p1);
        ArgumentNullException.ThrowIfNull(p2);
        if (!p1.Spouses.Contains(p2) && !p2.Spouses.Contains(p1)) { throw new ArgumentException(string.Format("{0} must be married to {1} to procreate with them.", arg0: p1.Name, arg1: p2.Name)); }
        Person baby = new()
        {
